...
color_list = [0xd0021b, 0xf8e71c, 0xf8e71c, 0x7fc538, 0x50e3c2, 0x1b79e7, 0x9013fe, 0xbd10e0]
...
async def on_command_error(ctx, error):
    if isinstance(error, discord.ext.commands.errors.CommandNotFound):
        embed = discord.Embed(title="Error", description="El comando que intentas usar no existe!.", colour= discord.Colour(random.choice(color_list)))
        await ctx.send(embed=embed)
    if isinstance(error, discord.ext.commands.errors.MemberNotFound):
        embed = discord.Embed(title="Error", description="No se ha encontrado al usuario.", colour= discord.Colour(random.choice(color_list)))
        await ctx.send(embed=embed)
    if isinstance(error, discord.ext.commands.errors.MissingRequiredArgument):
        embed = discord.Embed(title="Error", description="Por favor declara los argumentos requeridos.", colour= discord.Colour(random.choice(color_list)))
        await ctx.send(embed=embed)
    if isinstance(error, Exception):
        print(error)
    if isinstance(error, discord.ext.commands.MissingPermissions):
        embed = discord.Embed(title="Error", description="No tienes los permisos necesarios para ejecutar el comando.", colour= discord.Colour(random.choice(color_list)))
        await ctx.send(embed=embed)
...
@client.command()
async def ping(ctx):
    embed = discord.Embed(colour=discord.Colour(random.choice(color_list)),description="La latencia del bot es: " + f"**{round(client.latency*1000)}**ms")
    await ctx.send(embed=embed)
#########################/
...
@client.command()
async def help(ctx):
    embed = discord.Embed(title="Comandos", colour=discord.Colour(random.choice(color_list)))
...
async def schonkcreate(ctx):
    ...
    embed = discord.Embed(colour=discord.Colour(random.choice(color_list)), description=f"**{str(choose)+str(choose1)+str(choose2)}**")
    await ctx.send(embed=embed)
    choices.close()
    choices1.close()
    choices2.close()
...
async def achometro(ctx):
    ...
    if ctx.message.author.id in idlist:
        x = 100
        spembed = discord.Embed(colour=(0x7ed321), title="Acho que tu nivel de Acho es:", description="**100%**")
    ...
    embed = discord.Embed(colour=discord.Colour(random.choice(color_list)), title="Acho que tu nivel de Acho es:", description=f"{''.join(rangelist)}   **{x}%**\n{''.join(rangelist2)}\n{''.join(rangelist3)}")
...
async def serverinfo(ctx):
    embed = discord.Embed(title=f"🖥️ {ctx.guild.name}", colour=discord.Colour(random.choice(color_list)))
...
async def userinfo(ctx, user: discord.Member = None):
    ...
    embed = discord.Embed(title=user.name + "#" + str(user.discriminator), colour=discord.Colour(random.choice(color_list)))
...
async def avatar(ctx, user: discord.Member = None):
    if user == None:
        user = ctx.message.author
    embed = discord.Embed(colour=discord.Colour(random.choice(color_list)))
...
async def emojinotfound_error(ctx, error):
    if isinstance(error, discord.ext.commands.EmojiNotFound):
        embed = discord.Embed(title="Error", description="No se ha encontrado el emoji.", colour= discord.Colour(random.choice(color_list)))
        await ctx.send(embed=embed)
#########################/
...
async def ban(ctx, user: discord.User, *, reason = None):
    if user == ctx.message.author:
        embed = discord.Embed(title="¡Ey!", description=f":warning: {user.mention} No puedes autobanearte :warning:", colour= discord.Colour(random.choice(color_list)))
        await ctx.channel.send(embed=embed)
        return
    else:
        embed = discord.Embed(title="You're going to brazil", description=f"El usuario {user.mention} ha sido correctamente enviado a Brasil", colour= discord.Colour(random.choice(color_list)))
...
async def unban(ctx, user: discord.User):
    embed = discord.Embed(colour= discord.Colour(random.choice(color_list)), description=f"Has sido desbaneado de **{ctx.guild.name}**", title="Atención")
    embed2 = discord.Embed(colour= discord.Colour(random.choice(color_list)), description=f"El usuario {user.mention} ha vuelto de brasil")
...
async def hola(ctx, *, member: discord.Member = None):
    ...
    embed = discord.Embed(colour=discord.Colour(random.choice(color_list)), title="¡Hola!",description=f"{member.mention}", clearmention = False)
...
async def google(ctx, * ,search1 = None):
    if search1 == None:
        embed = discord.Embed(title="Error", description=f"{ctx.message.author.mention} Haz una búsqueda.", colour= discord.Colour(random.choice(color_list)))
        await ctx.channel.send(embed=embed)
    else:
        ...
        embed = discord.Embed(title=search, description="Resultados:", colour= discord.Colour(random.choice(color_list)))
